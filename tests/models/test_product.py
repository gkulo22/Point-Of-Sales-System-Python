...
class TestProductModels(unittest.TestCase):
    def test_product_get_price_without_discount(self) -> None:
        p = Product(id="p1", name="Test Product",
                    barcode="123456", price=100.0, discount=None)
        self.assertEqual(p.get_price(), 100.0)

    def test_product_get_price_with_discount_value(self) -> None:
        p = Product(id="p2", name="Test Product 2",
                    barcode="654321", price=100.0, discount=80.0)
        self.assertEqual(p.get_price(), 100.0)

    def test_product_decorator_get_price(self) -> None:
        p = Product(id="p3", name="Test Product 3",
                    barcode="111111", price=200.0, discount=None)
    ...
    def test_discounted_product_get_price(self) -> None:
        p = Product(id="p4", name="Test Product 4", barcode="222222",
                    price=100.0, discount=None)
